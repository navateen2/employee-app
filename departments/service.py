...
async def create(db:AsyncSession,body)->Department:
    ...
    department = await repository.create(db,body.name.strip(),password_hash=hashed)
    return department

async def all(db:AsyncSession)->list[dict]:
    return await repository.all(db)
    
async def update_department(department_id: int,name:str, db: AsyncSession)->Department:
    return await repository.update_employee(department_id,name.strip(),db)
